...
fun main(){
    ...
    println(data.flatMap { it.value }.filter { it>0 }.average()) //1.

    println(data.filterValues{ it.isNullOrEmpty()or it.any{number->number<0}}.keys.joinToString()) //2.

    println(data.flatMap { it.value }.filter { it<0 }.count()) //3.
